...
!
  ...
  !  close(21)
    ...
    call system('rm -f parafile *.temp force2 mmen2 mmen4*')
...
subroutine map_g09_cns(gjffile, num_gjf, pdbfile, num_pdb, map_list)
  ...
  backspace(52)
  do i = 1, num_pdb 
      read(52,'(A30,3F8.3,A26)') c1, coord_cns(1:3,i), c2
  enddo
  close(52)
  ...
  do i = 1, num_pdb
      ...
      if (pos_temp == 0) then
          write(*,'(I5,3F14.8)') i, coord_temp(1:3)
          number_0 = number_0 + 1
      endif
      map_list(i) = pos_temp
  enddo
...
subroutine lanch_external(gjffile, num_atom)
  ...
  elseif (method_num == 0 .or. method_num == 2) then
    ...
    INQUIRE(FILE = "bindividual.inp", EXIST = alive)
    call system('rm -f mmen4*')
    IF (alive) THEN
...
! Gaussian program
  ...
  if (coordfile == 1) then
    ...
    do i = 1, num_atom
      read(28,'(A30,3F8.3,A26)') c, coord_pdb(1:3,i), c1
      
      !write(*,*) c, coord_pdb(1:3,i), c1
      j = gjf_pdb_map(i)
...
subroutine update_pdb(num_atom, pdbfile, coords)
  ...
  CHARACTER(len=30), DIMENSION(2,num_atom) :: pdblines
  ...
  do i = 1, num_atom
    read(28,'(A30,3F8.3,A26)') pdblines(1,i), coord_pdb(1:3,i), pdblines(2,i)
    !write(*,*) pdblines(1,i), coord_pdb(1:3,i), pdblines(2,i)
  enddo
  close(28)
...
!
    ...
    write(29, '(A30,3F8.3,A26)') pdblines(1,i), xyz(:), pdblines(2,i)
    write(31, '(A30,3F8.3,A26)') pdblines(1,i), xyz1(:), pdblines(2,i)
  enddo
  write(29, *) 'END'
  write(29, *)
  write(31, *) 'END'
